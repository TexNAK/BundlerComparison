...
%		\subsection{Bundling Tool}
%			Für die Beantwortung der Forschungsfrage \ref{question1} stehen zwei Tools zur Auswahl:
%			\begin{enumerate}
%				\item ParcelJS\footnote{\url{https://parceljs.org}}
%				\item Webpack\footnote{\url{https://webpack.js.org}}
%			\end{enumerate}
%			Dabei wird im Rahmen dieser Arbeit das Tool Webpack genutzt, da weitaus mehr Dokumentation zur Verfügung steht. Die Wahl des Tools ist dabei nur von bedingter Relevanz für das Endergebnis, da ein Großteil der analysierten Prozesse durch externe Tools wie z.B. BabelJS\footnote{https://babeljs.io} oder UglifyJS\footnote{http://lisperator.net/uglifyjs/} durchgeführt wird, welche von den \Glspl{bundler} aufgerufen werden.
%    	\subsection{Hypothese}
%    		\paragraph{Variable 1} Beeinflussung der Build-Dauer durch Änderung der Konfiguration
%    		\paragraph{Variable 2} Relevanz des Buildzeitverhalten
			% TODO Variablen erklären
	...
	\section{Analyse der Faktoren mit Auswirkung auf das Buildzeitverhalten}
		\subsection{Ablauf des Build}
			% TODO Research what exactly happens during the build (tree building and such)
		\subsection{Methodik}
			...
			\subsubsection{Beispiel-Projekt}
				...
				\paragraph{Bundling Tool} In vielen Projekten wird die zu nutzende Technologie meist von Projektleitern oder Kunden vorgeschrieben. Entsprechend wurde für dieses Projekt der am weitesten verbreitete \Gls{bundler} \emph{Webpack} verwendet. % TODO Cite Google Trends or other source for webpack popularity https://trends.google.com/trends/explore?geo=US&q=parcel%20js,webpack
				
				\paragraph{React} Bei der Entwicklung von \Glspl{app} werden häufig Frameworks wie z.B. jQuery, React, Angular oder VueJS verwendet. In diesem Projekt wird \emph{React} aufgrund seines guten Rufs in der Entwickler-Community und den vorhandenen Kenntnissen des Authors verwendet. % TODO Cite Stackoverflow dev survey
		...
		\subsection{Durchführung}
			Who knows what'll happen?! % TODO Write this!
